    ...
    <DashboardLayout>
      <main className="flex-1 overflow-y-auto bg-background-light dark:bg-background-dark">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-6 md:p-10 pb-6">
          <div className="flex-1 max-w-2xl">
            <div className="relative">
              ...
              <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-primary/5 border border-primary/10 rounded-lg py-2.5 pl-10 pr-4 text-sm text-slate-200 focus:ring-1 focus:ring-primary focus:border-primary outline-none transition-all placeholder:text-slate-600" placeholder="Search transactions, amounts or IDs..." />
            </div>
          </div>
        ...
        <div className="px-6 md:px-10 pb-10">
          <div className="mb-8">
            <h2 className="text-3xl font-black text-slate-100 tracking-tight">Transactions Management</h2>
            <p className="text-slate-400 mt-1">Monitor all platform transactions and payments</p>
          </div>
          ...
          <div className="bg-primary/5 border border-primary/10 rounded-xl overflow-hidden shadow-2xl">
            <div className="px-6 py-5 border-b border-primary/10 flex items-center justify-between bg-primary/5">
              <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2">
                <span className="material-symbols-outlined text-primary">list_alt</span>Transactions List
              </h3>
              <div className="flex items-center gap-3">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-primary/5">
                  {loading ? (
                    <tr><td className="px-6 py-12 text-center" colSpan={6}><p className="text-slate-400">Loading transactions...</p></td></tr>
                  ) : transactions.length === 0 ? (
                    <tr><td className="px-6 py-4" colSpan={6}><p className="text-sm text-slate-400 text-center">No transactions yet</p></td></tr>
                  ) : (
                    transactions.map((tx) => (
                      <tr key={tx.id} className="hover:bg-primary/10 transition-colors group">
                        <td className="px-6 py-4"><span className="text-xs font-mono font-bold text-primary px-2 py-1 bg-primary/10 rounded">{tx.id.substring(0, 8).toUpperCase()}</span></td>
                        <td className="px-6 py-4"><span className="text-sm font-semibold text-slate-200">{tx.is_spend ? 'Spend' : 'Earn'}</span></td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${
                            tx.status === 'synced' ? 'bg-primary/20 text-primary border border-primary/30' : 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/30'
                          }`}>
                            <span className={`size-1.5 rounded-full ${tx.status === 'synced' ? 'bg-primary' : 'bg-yellow-400'}`}></span>
                            {tx.status}
                          </span>
                        </td>
                        <td className="px-6 py-4"><span className="text-sm font-bold text-slate-200">R{parseFloat(tx.purchase_amount || 0).toFixed(2)}</span></td>
                        <td className="px-6 py-4"><span className="text-sm text-slate-400">{new Date(tx.created_at).toLocaleDateString()}</span></td>
                        <td className="px-6 py-4">
                  ...
                  <tr className="bg-primary/5">
                    <td className="px-6 py-3 text-center" colSpan={6}><p className="text-[10px] text-slate-600 font-medium uppercase tracking-widest">Showing {transactions.length} of {transactions.length} total records</p></td>
                  </tr>
                </tbody>
              </table>
            </div>
