...
interface Invoice {
  id: string; partner_id: string; invoice_month: string;
...
interface Partner { id: string; name: string; bank_name: string; bank_account: string; account_holder: string; status: "active" | "suspended" }
...
export function PartnerInvoice() {
  ...
  const loadInvoiceData = async () => {
    ...
    try {
      ...
      if (partnerDetails) setPartner(partnerDetails);
      const currentMonth = new Date().toISOString().slice(0, 7);
      const { data: invoices } = await supabase.from("monthly_invoices").select("*").eq("partner_id", parsedPartner.id).order("invoice_month", { ascending: false }).limit(4);
      if (invoices?.length) {
  ...
  const copyPaymentInfo = () => {
    if (!Partner || !currentInvoice) return;
    navigator.clipboard.writeText(`Bank: ${partner.bank_name}\nAccount: ${partner.bank_account}\nHolder: ${partner.account_holder}\nRef: ${currentInvoice.eft_reference}\nAmount: R${currentInvoice.total_due.toFixed(2)}`);
      ...
      <header className="page-header">
        <div style={{ maxWidth: '56rem', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <p style={{ fontSize: '0.8125rem', color: 'rgba(255,255,255,0.7)', marginTop: '2px' }}>{partner?.name}</p>
          </div>
        ...
        <div style={{ maxWidth: '56rem', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          {currentInvoice ? (
                ...
                <div style={{ background: 'var(--blue-light)', borderRadius: '12px', padding: '1.25rem', border: '1px solid #dce8f5' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.875rem', marginBottom: '1rem' }}>
                    {[
                      ...
                      { label: 'Account Holder', value: partner?.account_holder },
                      { label: 'Account Number', value: partner?.bank_account },
            ...
            <div className="card" style={{ textAlign: 'center', padding: '3rem 1.5rem' }}>
              ...
              <button onClick={() => navigate("/partner/dashboard")} className="btn btn-primary" style={{ borderRadius: '10px' }}>← Back to Dashboard</button>
            </div>
          )}
        </div>
      </main>
